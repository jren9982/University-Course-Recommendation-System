    ...
    <div class="table-wrapper">
            ...
            <div class="row">
                ...
                <form class="d-flex" action="admin_cca_search.php" method="post">
                    <input class="form-control mr-sm-2" type="text" name="ccasearch" value="<?php if(isset($_POST['ccasearch'])){echo $_POST['ccasearch'];} ?>" id="ccasearch" class="form-control"
                           size="23" placeholder="Search CCA...">
        ...
        <div id="container">
            <table class="table table-bordered">
                ...
                <tbody>
                <?php
                if (isset($_POST['ccasearch'])) {

                $search = $_POST['ccasearch'];
                if ($search == "") {
                    header("Location: cca_admin.php");
                    exit();
                }
                $sql = mysqli_query($conn, "SELECT c.cca_id, l.uni_name, cat.category_name, c.cca_name FROM uni_cca c, uni_list l, uni_cca_categories cat WHERE l.uid = c.uid AND cat.category_id = c.category_id AND c.cca_name LIKE'%$search%'");
                if ($sql->num_rows > 0) {
                    while ($row = mysqli_fetch_assoc($sql)) {
                        ?>
                        <tr>
